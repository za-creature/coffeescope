...
(function() {
  ...
  module.exports = ScopeLinter = class ScopeLinter {
    ...
    constructor() {
      ...
      this.visitImportDeclaration = this.visitImportDeclaration.bind(this);
    ...
    visitImportDeclaration(node) {
      ...
      if (node.clause != null) {
        if (node.clause.defaultBinding != null) {
          this.scope.identifierWritten(node.clause.defaultBinding.identifier, node, 'Variable');
        }
        if (node.clause.namedImports != null) {
          // ImportSpecifierList
          if (node.clause.namedImports.specifiers) {
            ref = node.clause.namedImports.specifiers;
            for (i = 0, len = ref.length; i < len; i++) {
              specifier = ref[i];
              this.scope.identifierWritten(specifier.identifier, node, 'Variable');
            }
          // ImportNamespaceSpecifier
          } else if (node.clause.namedImports.alias) {
            this.scope.identifierWritten(node.clause.namedImports.alias.value, node, 'Variable');
          }
        }
      }
      return void 0;
    ...
    visitExportNamedDeclaration(node) {
      ...
      if (node.clause.specifiers) {
        ...
        switch (node.clause.constructor.name) {
          case 'Assign':
            this.scope.identifierRead(node.clause.variable.value, node.clause.value);
            break;
          case 'Class':
            this.scope.identifierRead(node.clause.variable.base.value, node.clause.variable);
        }
      }
      node.eachChild(this.visit);
      return void 0;
